...
//! let result = fallback.provide().await?;
//! ```
//!
    ...
    E: Send,
{
    ...
    async fn provide(&self) -> Result<T, Self::Error> {
        match self.primary.provide().await {
            Ok(value) => Ok(value),
            Err(_) => self.fallback.provide().await,
        }
    }
}
...
mod tests {
    ...
    impl Provider<String> for FailingProvider {
        ...
        async fn provide(&self) -> Result<String, Self::Error> {
    ...
    async fn test_fallback_uses_first_success() {
        ...
        let result = provider.provide().await.unwrap();
    ...
    #[tokio::test]
    async fn test_chained_fallbacks() {
