...
- **Linux x86_64 / arm64**: a portable `.tar.gz` (binary + `.desktop`/icon/metainfo +
  ...
  Raspberry Pi 5. Kept in lockstep with the x86_64 job's `ubuntu-latest` (currently 24.04) by
  hand — GitHub has no `ubuntu-latest-arm` alias, so the arm label needs a manual bump whenever
  `ubuntu-latest` moves to a new default.
