...
settings-scan-concurrency = Concurrency
settings-scan-concurrency-description = Number of files processed in parallel during a scan.
settings-scan-tar-single-pass = Single-Pass Tar Extraction
settings-scan-tar-single-pass-description = Extract all documents from a tar archive in one pass, using temporary disk space, instead of re-reading the archive for every document. Faster for large archives.
...
settings-server-network-section = Network
