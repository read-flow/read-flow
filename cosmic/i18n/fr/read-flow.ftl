# The keys in each section (separated by a comment) are sorted alphabetically

app-title = Read Flow
about = À propos
shortcuts = Raccourcis clavier
shortcuts-section-documents = Documents
shortcuts-elsewhere = Ailleurs
shortcuts-escape-back-or-close = Retour / fermer (détails, préférences, journal du serveur, visionneuse d'images)
shortcuts-open-this-panel = Ouvrir ce panneau
actions = Actions
view-options = Options d'affichage

# Preferences page
preferences-page-title = Préférences
preferences-back = ← Préférences
preferences-appearance-section = Apparence
preferences-appearance-section-description = Préférences du lecteur de documents et d'affichage
preferences-library-section = Bibliothèque
preferences-library-section-description = Emplacement du fichier de base de données
preferences-downloads-section = Téléchargements
preferences-downloads-section-description = Dossier par défaut pour les documents téléchargés
preferences-scanning-section = Analyse
preferences-scanning-section-description = Répertoires d'analyse automatique et types de fichiers pris en charge
preferences-sources-section = Sources de documents
preferences-sources-section-description = Serveurs distants pour synchroniser les documents
preferences-server-section = Serveur
preferences-server-section-description = Accès au serveur et utilisateurs autorisés
preferences-online-library-section = Bibliothèque en ligne
preferences-online-library-section-description = Gérer les catalogues OPDS intégrés et personnalisés
preferences-privacy-section = Confidentialité
preferences-privacy-section-description = Mode privé et balises cachées
context = Contexte
repository = Dépôt
scan = Analyser
check-missing = Vérifier les fichiers manquants
check-missing-dialog-title = Fichiers manquants
check-missing-dialog-body = Les fichiers suivants ne sont plus accessibles sur le disque :
check-missing-no-missing = Aucun fichier manquant trouvé.
check-missing-purge = Purger
check-missing-cancel = Annuler
scan-progress-scanning = Analyse en cours… { $discovered } trouvés, { $processed } traités
scan-progress-completed = Dernière analyse : { $discovered } trouvés, { $processed } traités, { $errors } erreurs
scan-report-title = Rapport d'analyse
scan-report-summary = { $added } ajoutés, { $updated } mis à jour, { $errors } erreurs
scan-report-close = Fermer
view = Affichage
nav-open-documents = Ouverts

# Generic messages
generic-error = 🛑 Erreur: { $error }

# Color picker
color-picker-copied = Copié dans le presse-papiers
color-picker-copy = Copier dans le presse-papiers
color-picker-hex = Hex
color-picker-recent = Couleurs récentes
color-picker-rgb = RVB

# File details
document-details-close = Retour
document-details-document-no-longer-accessible = Document n'est plus accessible
document-details-metadata-date = Date
document-details-metadata-identifier = Identifiant
document-details-metadata-language = Langue
document-details-metadata-publisher = Éditeur
document-details-metadata-subject = Sujet
document-details-no-covers = No covers available
document-details-no-sources = Aucune source disponible
document-details-select-cover = Select Cover
document-details-change-thumbnail = Changer la vignette
document-details-change-thumbnail-tooltip = Choisir une page à utiliser comme vignette
document-details-thumbnail-page-label = Page { $current } sur { $total }
document-details-thumbnail-loading = Chargement…
document-details-thumbnail-trim = Rogner les marges blanches
document-details-thumbnail-padding = Marge intérieure : { $padding }px
document-details-thumbnail-margins = Exclure avant le recadrage (ex. un numéro de page)
document-details-thumbnail-margin-top = Haut : { $margin }px
document-details-thumbnail-margin-bottom = Bas : { $margin }px
document-details-thumbnail-margin-left = Gauche : { $margin }px
document-details-thumbnail-margin-right = Droite : { $margin }px
document-details-thumbnail-save = Enregistrer
document-details-thumbnail-cancel = Annuler
document-details-open-file = Ouvrir le Fichier
document-details-select-status = Sélectionner le statut
document-details-status = Statut
document-details-tags = Étiquettes
document-details-document-meta-section = Infos du document
document-details-document-meta-edit = Modifier les infos
document-details-document-meta-save = Enregistrer
document-details-document-meta-cancel = Annuler
document-details-document-meta-type = Type
document-details-document-meta-type-none = Non défini
document-details-document-meta-title = Titre
document-details-document-meta-subtitle = Sous-titre
document-details-document-meta-authors = Auteurs
document-details-document-meta-authors-add = Ajouter un auteur
document-details-document-meta-description = Description

# Tag editor
tag-editor-add = Ajouter
tag-editor-loading-tags = Chargement des étiquettes...
tag-editor-no-tags = Aucune étiquette
tag-editor-remove-tag = Supprimer l'étiquette
tag-editor-select-tag = Sélectionner une étiquette

# Document details
document-details-copy-path = Copier le chemin
document-details-delete-copy = Supprimer la copie
document-details-delete-copy-confirm-body = Voulez-vous vraiment supprimer cette copie du document ? Le fichier sera retiré de cet appareil et cette copie ne pourra plus être ouverte. Cette action ne peut pas être annulée.
document-details-delete-copy-confirm-cancel = Annuler
document-details-delete-copy-confirm-delete = Supprimer
document-details-delete-copy-confirm-title = Supprimer la copie ?
document-details-done-editing-sources = Terminer la modification
document-details-download-to-local = Télécharger en local
document-details-edit-sources = Modifier les sources
document-details-send-to-missing = Envoyer vers
document-details-source-added = Ajouté le { $date }
document-details-source-local = Local
document-details-sources = Formats
document-details-sync-to-all-sources = Synchroniser avec toutes les sources
document-details-upload-to = Envoyer vers { $host }
document-details-remove-format = Supprimer le format { $format }
document-details-remove-format-tooltip = Supprimer le format { $format } et toutes ses copies
document-details-remove-format-confirm-title = Supprimer le format { $format } ?
document-details-remove-format-confirm-body = Chaque copie { $format } de ce document sera définitivement supprimée, y compris sa progression de lecture et ses étiquettes. Cette action ne peut pas être annulée.
document-details-remove-format-confirm-files = Fichiers à supprimer :
document-details-remove-format-confirm-remove = Supprimer { $count } fichiers
document-details-remove-format-confirm-cancel = Annuler

# Document list
document-list-all-sources = Toutes les sources
document-list-all-statuses = Tous les statuts
document-list-clear-all-tag-filters = Effacer tous les filtres d'étiquettes
document-list-clear-filter = Effacer le filtre
document-list-deselect-all = Tout désélectionner
document-list-empty-description = Configurez les répertoires d'analyse dans les Paramètres pour que Read Flow découvre vos documents, puis lancez une analyse pour les ajouter à votre bibliothèque.
document-list-empty-title = Aucun document trouvé
document-list-filter-by-source = Filtrer par Source
document-list-filter-by-status = Filtrer par statut de lecture
document-list-filter-by-tags = Filtrer par étiquettes
document-list-filter-by-type = Filtrer par type de fichier
document-list-filtering = Filtrage...
document-list-go-to-settings = Ouvrir les paramètres
document-list-keyboard-shortcuts = Raccourcis clavier
document-list-loading = Chargement
document-list-merge = Fusionner
document-list-merge-body = Choisissez le document qui conservera ses métadonnées. Toutes les sources de fichiers des autres documents lui seront transférées.
document-list-merge-cancel = Annuler
document-list-merge-candidate-summary = { $files } fichier(s) · { $tags } étiquette(s)
document-list-merge-candidate-tags = { $count } étiquettes
document-list-merge-confirm = Fusionner
document-list-merge-confirm-body = « { $winner } » conservera ses métadonnées. { $count } autre(s) document(s) ({ $losers }) seront supprimés ; leurs fichiers seront déplacés vers « { $winner } ». Cette action ne peut pas être annulée.
document-list-merge-title = Fusionner les documents
document-list-common-tags-label = Sur tous les documents sélectionnés
document-list-partial-tags-label = Également sur certains documents sélectionnés
document-list-partial-tag-remove-tooltip = Retirer des documents sélectionnés qui l'ont
document-list-no-files = Aucun fichier ne correspond aux filtres actuels
document-list-open-document-details = Ouvrir
document-list-options-title = Options de liste de fichiers
document-list-pick-format-cancel = Annuler
document-list-pick-source-local = Local
document-list-pick-source-title = Ouvrir le fichier
document-list-run-scan = Lancer l'analyse
document-list-search-mode = Mode de recherche
document-list-search-mode-fuzzy = Flou
document-list-search-mode-regex = Regex
document-list-search-placeholder = Rechercher des fichiers...
document-list-select-all = Tout sélectionner
document-list-selection-count = { $selected } de { $total } sélectionnés
document-list-shortcut-first-page = Aller à la première page
document-list-shortcut-last-page = Aller à la dernière page
document-list-shortcut-next-page = Aller à la page suivante
document-list-shortcut-previous-page = Aller à la page précédente
document-list-shortcut-toggle-search-mode = Basculer le mode de recherche (Flou / Regex)
document-list-sort-added = Date d'ajout
document-list-sort-by = Trier par
document-list-sort-filename = Nom
document-list-sort-size = Taille
document-list-sort-status = Statut
document-list-sort-title = Titre
document-list-sort-type = Type

# Page messages
page-not-found = ⚠ Non trouvé

# Language
language-dutch = Nederlands
language-english = English
language-french = Français
settings-language-section = Langue
settings-language = Langue
settings-language-description = Langue d'affichage de Read Flow.
settings-language-follow-system = Suivre le système

# Pagination
pagination-first = Premier
pagination-last = Dernier
pagination-next = Suivant
pagination-page-of-total = Page { $page } sur { $total }
pagination-prev = Précédent

# Sources
sources-add-button = Ajouter la source
sources-add-section-title = Ajouter une nouvelle source
sources-edit-section-title = Modifier la source
sources-submit-button = Enregistrer la source
sources-add-unavailable-warning = Cette source semble inaccessible. Vous pouvez quand même l'ajouter — elle apparaîtra comme inaccessible jusqu'à ce qu'elle soit disponible.
sources-authorization-token = Le jeton d'autorisation
sources-authorization-token-placeholder = Entrer le jeton d'autorisation...
sources-delete-confirm-body = Êtes-vous sûr de vouloir supprimer cette source ? Cette action ne peut pas être annulée.
sources-delete-confirm-cancel = Annuler
sources-delete-confirm-delete = Supprimer
sources-delete-confirm-title = Supprimer la source
sources-empty-state = Aucune source distante configurée
sources-error-close = Fermer
sources-error-title = Une erreur s'est produite
sources-invalid-url = URL non valide
sources-loading-state-loading = Chargement...
sources-loading-state-new = Initialisation
sources-section-title = Sources distantes
sources-status-checking = Vérification de l'accessibilité...
sources-status-reachable = Accessible
sources-status-unknown = Statut inconnu
sources-status-unreachable = Inaccessible
sources-url = l'URL distante
sources-url-placeholder = Entrer l'URL distante
sources-user-id = ID utilisateur
sources-user-id-placeholder = Entrer l'ID utilisateur...

# Settings page
settings-add-directory = Ajouter un répertoire
settings-cancel-edit = Annuler
settings-client-download-folder = Dossier de téléchargement
settings-client-download-folder-description = Dossier où les fichiers extraits des sources distantes sont enregistrés
settings-client-section = Client
settings-client-section-description = Configurez le dossier local où les fichiers téléchargés depuis des sources distantes sont enregistrés.
settings-context-title = Paramètres de confidentialité
settings-database-location = Emplacement de la base de données
settings-database-location-description = Le fichier SQLite qui stocke votre catalogue de documents et les métadonnées. Redémarrez l'application après avoir modifié ce chemin.
settings-database-section = Base de données
settings-database-section-description = Configurez l'emplacement de stockage de votre catalogue de documents.
settings-directory-action = Action
settings-directory-action-ignore = Ignorer
settings-directory-action-ignore-label = Ignorer
settings-directory-action-scan = Scanner
settings-directory-action-scan-label = Scanner
settings-directory-inherit = Hériter les paramètres aux sous-répertoires
settings-directory-path = Chemin du répertoire
settings-directory-tags = Tags
settings-edit-directory = Modifier le répertoire
settings-failed-to-load-tags = Échec du chargement des tags
settings-no-directory-tags = Aucune étiquette configurée
settings-no-private-tags = Aucun tag privé configuré
settings-online-library-add-catalog = Ajouter un catalogue
settings-online-library-add-catalog-title = Nouveau catalogue
settings-online-library-builtin-catalogs = Catalogues intégrés
settings-online-library-catalog-name = Nom
settings-online-library-catalog-name-placeholder = Entrer le nom du catalogue...
settings-online-library-catalog-search-url = URL de recherche
settings-online-library-catalog-search-url-placeholder = https://exemple.com/opds/search?q=<termes de recherche>
settings-online-library-custom-catalogs = Catalogues personnalisés
settings-online-library-custom-catalogs-description = Ajoutez vos propres catalogues OPDS pour les rechercher en plus des catalogues intégrés.
settings-online-library-edit-catalog = Modifier le catalogue
settings-online-library-submit-catalog = Enregistrer le catalogue
settings-online-library-delete-catalog-confirm-title = Supprimer le catalogue
settings-online-library-delete-catalog-confirm-body = Êtes-vous sûr de vouloir supprimer ce catalogue ? Cette action ne peut pas être annulée.
settings-online-library-delete-catalog-confirm-remove = Supprimer
settings-remove-directory = Supprimer le répertoire
settings-remove-directory-confirm-body = Êtes-vous sûr de vouloir supprimer ce répertoire de l'analyse ? Cette action ne peut pas être annulée.
settings-remove-directory-confirm-remove = Supprimer
settings-remove-directory-tag = Supprimer l'étiquette
settings-remove-private-tag = Supprimer le tag privé
settings-revert = Annuler les modifications
settings-save = Enregistrer les paramètres
settings-save-directory = Enregistrer le répertoire
settings-save-error = Erreur lors de l'enregistrement des paramètres
settings-saved = Paramètres enregistrés avec succès
settings-saving = Enregistrement...
settings-scan-description = Read Flow indexe les documents trouvés dans les répertoires ci-dessous. Ajoutez un répertoire, définissez son action sur Scanner, puis lancez une analyse pour remplir votre bibliothèque.
settings-scan-directories-section = Répertoires de scan
settings-scan-dry-run = Mode d'exécution à sec
settings-scan-dry-run-description = Lorsqu'il est activé, les analyses découvrent les fichiers mais ne les ajoutent pas à votre bibliothèque. Utile pour prévisualiser ce qui serait indexé.
settings-scan-concurrency = Concurrence
settings-scan-concurrency-description = Nombre de fichiers traités en parallèle pendant une analyse.
settings-scan-tar-single-pass = Extraction tar en une passe
settings-scan-tar-single-pass-description = Extraire tous les documents d'une archive tar en une seule passe, en utilisant de l'espace disque temporaire, au lieu de relire l'archive pour chaque document. Plus rapide pour les grandes archives.
settings-scan-file-types-description = Seuls les fichiers dont l'extension est activée seront pris en compte lors d'une analyse.
settings-scan-file-types-deselect-all = Tout désélectionner
settings-scan-file-types-section = Types de fichiers
settings-scan-file-types-select-all = Tout sélectionner
settings-scan-section = Analyse
settings-scan-section-description = Configurez les répertoires à indexer et les types de fichiers à inclure.
settings-select-directory-tag = Sélectionner une étiquette...
settings-select-private-tag = Sélectionner un tag...
settings-server-add-authorized-user = Ajouter un utilisateur autorisé
settings-server-authorized-users = Utilisateurs autorisés
settings-server-authorized-users-description = Les clients autorisés à se connecter au serveur de cet appareil. Chaque entrée nécessite un identifiant utilisateur et une phrase de passe.
settings-server-description = Read Flow peut diffuser votre bibliothèque sur le réseau afin que d'autres appareils puissent se connecter et accéder à vos documents.
settings-server-download-folder = Dossier de téléchargement
settings-server-download-folder-description = Où les fichiers envoyés à cet appareil depuis des clients distants sont enregistrés.
settings-server-edit-authorized-user = Modifier l'utilisateur autorisé
settings-server-submit-authorized-user = Enregistrer l'utilisateur autorisé
settings-server-delete-authorized-user-confirm-title = Supprimer l'utilisateur autorisé
settings-server-delete-authorized-user-confirm-body = Êtes-vous sûr de vouloir supprimer cet utilisateur autorisé ? Cette action ne peut pas être annulée.
settings-server-delete-authorized-user-confirm-remove = Supprimer
settings-server-local-identity = Identité locale
settings-server-local-identity-example = Utile seulement si vous lisez aussi cette bibliothèque depuis la PWA ou un autre appareil. Exemple : choisir « jane » ici signifie que la progression de lecture et les étiquettes définies sur cet ordinateur apparaissent quand jane se connecte à la PWA sur son téléphone — et inversement.
settings-server-local-user = Utilisateur local
settings-server-local-user-description = Lorsque défini, la progression de lecture et les tags locaux de cet appareil sont enregistrés sous cet utilisateur autorisé, afin d'être repris par ce même utilisateur connecté à distance (par ex. depuis la PWA). Laissez vide pour garder l'accès local privé à cet appareil uniquement.
settings-server-local-user-none = Aucun (purement local)
settings-server-owner-role = Rôle Propriétaire
settings-server-owner-role-description = Donne accès au contenu privé (requis pour demander le mode privé à ce serveur).
settings-server-passphrase = La phrase de passe
settings-server-passphrase-placeholder = Entrer la phrase de passe...
settings-server-section = Serveur
settings-server-section-description = Partagez votre bibliothèque sur le réseau et gérez les appareils autorisés à se connecter.
settings-server-user-id = L'ID utilisateur
settings-server-user-id-placeholder = Entrer l'ID utilisateur...
settings-theme-accent = Couleur d'accent
settings-theme-accent-description = Couleur de mise en valeur des boutons, liens et sélections.
settings-theme-accent-blue = Bleu
settings-theme-accent-indigo = Indigo
settings-theme-accent-purple = Violet
settings-theme-accent-pink = Rose
settings-theme-accent-red = Rouge
settings-theme-accent-orange = Orange
settings-theme-accent-yellow = Jaune
settings-theme-accent-green = Vert
settings-theme-accent-warm-grey = Gris chaud
settings-theme-accent-custom = Couleur d'accent personnalisée
settings-theme-accent-default = Par défaut
settings-theme-background = Arrière-plan de la fenêtre
settings-theme-background-description = Couleur de base derrière tout le contenu. Laisser vide pour la dériver de l'apparence.
settings-theme-container-background = Arrière-plan des conteneurs
settings-theme-container-background-description = Couleur des cartes, boîtes de dialogue et autres conteneurs.
settings-theme-contrast-warning = Contraste faible : le texte peut être difficile à lire sur cet arrière-plan.
settings-theme-custom = Utiliser un thème personnalisé
settings-theme-custom-description = Personnaliser le thème de Read Flow sans modifier les paramètres système COSMIC.
settings-theme-dark = Sombre
settings-theme-density = Densité
settings-theme-density-description = Espacement entre les éléments de l'interface.
settings-theme-density-compact = Compacte
settings-theme-density-standard = Standard
settings-theme-density-spacious = Spacieuse
settings-theme-font = Police de l'interface
settings-theme-font-description = Police des menus, libellés et contrôles.
settings-theme-font-placeholder = Police système
settings-theme-monospace-font = Police à chasse fixe
settings-theme-monospace-font-description = Police des blocs de code et du texte préformaté.
settings-theme-monospace-font-placeholder = Police système
settings-theme-frosted = Verre dépoli
settings-theme-frosted-description = Arrière-plan de fenêtre translucide et flouté. Nécessite la prise en charge du flou par le compositeur (COSMIC sur Wayland).
settings-theme-frosted-strength = Intensité du flou
settings-theme-frosted-strength-low = Faible
settings-theme-frosted-strength-medium = Moyenne
settings-theme-frosted-strength-high = Forte
settings-theme-light = Clair
settings-theme-roundness = Arrondi
settings-theme-roundness-description = Degré d'arrondi des coins des boutons et conteneurs.
settings-theme-roundness-round = Arrondi
settings-theme-roundness-slightly-round = Légèrement arrondi
settings-theme-roundness-square = Carré
settings-theme-section = Thème
settings-theme-variant = Édition
settings-theme-variant-description = Une apparence claire et une apparence sombre sont enregistrées en même temps ; Read Flow bascule automatiquement entre les deux selon votre système. Choisissez ci-dessous laquelle modifier.
settings-ui-private-mode = Mode privé
settings-ui-private-mode-description = Lorsqu'il est activé, les documents portant un tag privé sont masqués dans la liste des documents.
settings-ui-private-tags = Tags privés
settings-ui-private-tags-description = Les documents portant l'un de ces tags sont masqués lorsque le mode privé est activé.
settings-ui-section = Confidentialité

# PDF viewer
pdf-viewer = PDF viewer
pdf-viewer-back = Retour
pdf-viewer-document-details = Détails du document
pdf-viewer-dual-page = Double page
pdf-viewer-dual-page-auto = Auto
pdf-viewer-dual-page-off = Désactivé
pdf-viewer-dual-page-on = Activé
pdf-viewer-epub-font-size = Taille de police (refluable)
pdf-viewer-keyboard-shortcuts = Raccourcis clavier
pdf-viewer-load-error = Impossible d'ouvrir le PDF : { $error }
pdf-viewer-loading = Chargement du PDF...
pdf-viewer-no-local-source = Aucune source locale disponible pour ce document
pdf-viewer-open-in-action = Ouvrir
pdf-viewer-open-in-external = Visionneuse externe (par défaut du système)
pdf-viewer-open-in-section = Ouvrir dans une autre visionneuse
pdf-viewer-shortcut-ctrl-scroll = Zoomer avec la souris
pdf-viewer-shortcut-fit-both = Ajuster largeur et hauteur
pdf-viewer-shortcut-fit-height = Ajuster la hauteur
pdf-viewer-shortcut-fit-width = Ajuster la largeur
pdf-viewer-shortcut-next-page = Page suivante
pdf-viewer-shortcut-previous-page = Page précédente
pdf-viewer-shortcut-zoom-in = Zoomer
pdf-viewer-shortcut-zoom-out = Dézoomer
pdf-viewer-shortcut-zoom-reset = Réinitialiser le zoom (100%)
pdf-viewer-show-thumbnails = Show thumbnails # TODO
pdf-viewer-theme-colors = Couleurs du thème
pdf-viewer-zoom = Zoom

# EPUB viewer
epub-viewer = EPUB viewer
epub-viewer-back = Retour
epub-viewer-document-details = Détails du document
epub-viewer-copy-code = Copier le code
epub-viewer-display = Affichage
epub-viewer-dual-page = Double page
epub-viewer-dual-page-auto = Auto
epub-viewer-dual-page-off = Désactivé
epub-viewer-dual-page-on = Activé
epub-viewer-font = Police
epub-viewer-font-size = Taille de police
epub-viewer-image-viewer-title = Image
epub-viewer-image-zoom = Zoom
epub-viewer-image-zoom-fit-both = Ajuster largeur et hauteur
epub-viewer-image-zoom-fit-height = Ajuster la hauteur
epub-viewer-image-zoom-fit-width = Ajuster la largeur
epub-viewer-image-zoom-in = Zoomer
epub-viewer-image-zoom-out = Dézoomer
image-viewer-keyboard-shortcuts = Raccourcis clavier
image-viewer-shortcut-zoom-in = Zoomer
image-viewer-shortcut-zoom-out = Dézoomer
image-viewer-shortcut-close = Fermer
epub-viewer-keyboard-shortcuts = Raccourcis clavier
epub-viewer-load-error = Impossible d'ouvrir l'EPUB : { $error }
epub-viewer-loading = Chargement de l'EPUB...
epub-viewer-no-local-source = Aucune source locale disponible pour ce document
epub-viewer-open-in-section = Ouvrir dans une autre visionneuse
epub-viewer-open-in-mupdf = Visionneuse MuPDF
epub-viewer-open-in-external = Visionneuse externe (par défaut du système)
epub-viewer-open-in-action = Ouvrir
epub-viewer-page-margin = Marge de page
epub-viewer-raw-html = Afficher le HTML brut (débogage)
epub-viewer-search = Rechercher
epub-viewer-search-match-count = { $current } sur { $total }
epub-viewer-search-next = Résultat suivant
epub-viewer-search-no-matches = Aucun résultat dans ce chapitre
epub-viewer-search-placeholder = Rechercher dans le chapitre...
epub-viewer-search-prev = Résultat précédent
epub-viewer-shortcut-next-chapter = Chapitre suivant
epub-viewer-shortcut-next-page = Page suivante
epub-viewer-shortcut-previous-chapter = Chapitre précédent
epub-viewer-shortcut-toggle-search = Basculer la recherche
epub-viewer-shortcut-close-search = Fermer la recherche
epub-viewer-shortcut-previous-page = Page précédente
epub-viewer-view-paginated = Vue paginée

# Tableau de bord
dashboard-page-title = Tableau de bord
dashboard-continue-reading = Continuer la lecture
dashboard-continue-reading-empty = Reprenez là où vous en étiez
dashboard-continue-reading-empty-hint = Ouvrez un document pour commencer à suivre votre progression de lecture.
dashboard-library-overview = Aperçu de la bibliothèque
dashboard-stat-documents = Documents
dashboard-stat-reading = En cours
dashboard-stat-completed = Terminés
dashboard-format-breakdown = Par format
dashboard-sources = Sources
dashboard-quick-actions = Actions rapides
dashboard-all-documents = Tous les documents
dashboard-welcome-title = Bienvenue dans Read Flow
dashboard-welcome-description = Votre bibliothèque de documents personnelle. Ajoutez des documents pour commencer.
dashboard-onboarding-step-scan-title = Configurer les répertoires d'analyse
dashboard-onboarding-step-scan-description = Indiquez à Read Flow où se trouvent vos documents sur le disque.
dashboard-onboarding-step-run-title = Lancer votre première analyse
dashboard-onboarding-step-run-description = Découvrez et indexez tous les documents dans vos répertoires configurés.
dashboard-onboarding-step-online-title = Parcourir les bibliothèques en ligne
dashboard-onboarding-step-online-description = Téléchargez des livres gratuits depuis Project Gutenberg et Standard Ebooks.
dashboard-onboarding-step-remote-title = Connecter une source distante
dashboard-onboarding-step-remote-description = Synchronisez avec une autre instance Read Flow sur votre réseau.
dashboard-action-go = Aller

# Bibliothèques en ligne
online-library-back-to-filters = ← Retour aux filtres
online-library-book-details = Détails du livre
online-library-catalog-all = Tous les catalogues
online-library-catalog-section-title = Catalogue
online-library-download = Télécharger
online-library-downloaded = Ajouté à la bibliothèque
online-library-downloading = Téléchargement…
online-library-empty-state = Entrez un terme de recherche pour trouver des livres
online-library-welcome-title = Découvrir des livres en ligne
online-library-welcome-subtitle = Recherchez des catalogues libres et ouverts dans le monde entier, puis téléchargez directement dans votre bibliothèque
online-library-hint-search-title = Rechercher
online-library-hint-search-body = Trouvez des livres par titre, auteur ou mot-clé dans tous vos catalogues connectés
online-library-hint-download-title = Télécharger
online-library-hint-download-body = Obtenez des livres en EPUB, PDF et d'autres formats en un clic
online-library-hint-library-title = Enrichir votre bibliothèque
online-library-hint-library-body = Les livres téléchargés sont automatiquement ajoutés à votre collection locale
online-library-layout-cards = Cartes
online-library-layout-compact = Compact
online-library-layout-section-title = Disposition
online-library-no-description = Aucune description disponible
online-library-no-results = Aucun résultat trouvé
online-library-page-title = Bibliothèques en ligne
online-library-search-button = Rechercher
online-library-search-placeholder = Rechercher des livres…
online-library-searching = Recherche en cours…

# Server page
server-log-page-title = Serveur
server-panel-title = Contrôle du serveur
server-start = Démarrer le serveur
server-stop = Arrêter le serveur
server-restart = Redémarrer
server-reload-config = Recharger la config
server-status-stopped = Le serveur est arrêté
server-status-stopped-detail = Démarrez-le pour accepter les connexions de vos autres appareils.
server-status-starting = Démarrage du serveur…
server-status-running = Le serveur est en cours d'exécution
server-status-running-detail = Accessible à { $address }
server-status-failed = Le serveur n'a pas pu démarrer
# Log viewer
server-log-min-level = Afficher
server-log-search = Rechercher dans les journaux…
server-log-empty = Rien à afficher pour l'instant
server-log-counts = { $errors } erreurs · { $warnings } avertissements
server-log-details-title = Entrée de journal
server-log-back = ← Retour
server-log-select-hint = Sélectionnez une entrée pour voir son message, ses détails et son contexte.
server-log-detail-target = Source
server-log-detail-message = Message
server-log-detail-fields = Détails
server-log-detail-spans = Contexte
log-level-trace = Trace et plus
log-level-debug = Débogage et plus
log-level-info = Info et plus
log-level-warn = Avertissements et erreurs
log-level-error = Erreurs uniquement
# Server preferences
settings-server-address = Adresse d'écoute
settings-server-address-description = Adresse IP sur laquelle le serveur écoute (p. ex. 127.0.0.1 ou 0.0.0.0)
settings-server-port = Port
settings-server-port-description = Port sur lequel le serveur écoute (0 = port libre automatique)
settings-server-start-on-launch = Démarrer le serveur au lancement
settings-server-start-on-launch-description = Démarrer automatiquement le serveur à l'ouverture de l'application
settings-server-restart-to-apply = Redémarrez le serveur pour appliquer la nouvelle adresse ou le nouveau port
settings-server-advanced = Avancé
settings-server-advanced-description = Origines CORS, limites de téléversement et TLS. Sans danger de laisser aux valeurs par défaut, sauf si le serveur est accessible au-delà de votre propre appareil.
# Server network/TLS preferences
settings-server-allowed-origins = Origines autorisées
settings-server-allowed-origins-description = Origines web autorisées par CORS, séparées par des virgules (vide = toutes)
settings-server-allowed-origins-placeholder = toutes les origines
settings-server-max-upload = Téléversement max (Mio)
settings-server-max-upload-description = Taille maximale de téléversement acceptée, en mébioctets
settings-server-network-section = Réseau
settings-server-tls-section = HTTPS (TLS)
settings-server-tls = Activer HTTPS (TLS)
settings-server-tls-description = Servir en HTTPS avec le certificat et la clé ci-dessous (redémarrage requis)
settings-server-tls-cert = Certificat TLS
settings-server-tls-cert-placeholder = /chemin/vers/fullchain.pem
settings-server-tls-key = Clé privée TLS
settings-server-tls-key-placeholder = /chemin/vers/privkey.pem
# Self-signed certificate generation
settings-server-generate-cert = Certificat TLS
settings-server-generate-cert-description = Générer un certificat pour ce serveur, signé par une autorité de certification locale (générée une fois puis réutilisée). Faites confiance à cette autorité une fois par appareil (voir « Certificat de l'autorité locale » ci-dessous) et chaque certificat généré ensuite est approuvé automatiquement, sans avertissement. Enregistrez pour appliquer.
settings-server-generate-cert-button = Générer
settings-server-open-ca-cert = Certificat de l'autorité locale
settings-server-open-ca-cert-description = Ouvrir le certificat racine de l'autorité de certification locale pour lui faire confiance sur cet appareil (par ex. l'importer dans Trousseau d'accès sur macOS). À faire une fois par appareil que vous souhaitez connecter sans avertissement de certificat.
settings-server-open-ca-cert-button = Ouvrir
# Activity history
activity-page-title = Activité
activity-refresh = Actualiser
activity-load-more = Charger plus
activity-summary = { $errors } en échec · { $interrupted } interrompues
activity-error = { $message }
activity-loading = Chargement de l'activité…
activity-empty = Rien d'enregistré pour le moment. Les mutations et analyses apparaîtront ici.
activity-select-hint = Sélectionnez une opération pour voir ses événements, son auteur et ses cibles.
activity-details-title = Opération
activity-dry-run = Essai à blanc
activity-detail-started = Début
activity-detail-completed = Fin
activity-detail-actor = Auteur
activity-detail-channel = Canal
activity-detail-status = Statut
activity-detail-error-code = Code d'erreur
activity-detail-counts = Compteurs
activity-detail-events = Événements
activity-detail-no-events = Aucun événement enregistré
activity-detail-related = Historique des documents liés
activity-op-scan = Analyse
activity-op-file-delete = Supprimer le fichier
activity-op-document-merge = Fusionner les documents
activity-op-document-content-removed = Retirer un contenu du document
activity-op-metadata-edit = Modifier les métadonnées
activity-op-tag-edit = Modifier les étiquettes
activity-op-status-edit = Changer le statut de lecture
activity-op-cover-edit = Changer la couverture
activity-op-missing-file-maintenance = Nettoyage des fichiers manquants
activity-channel-rest = API HTTP
activity-channel-cosmic = Application de bureau
activity-channel-cli = Ligne de commande
activity-channel-internal = Interne
activity-actor-user = Utilisateur { $id }
activity-actor-local = Cet appareil
activity-actor-local-session = Cet appareil (session)
activity-actor-system = Système
activity-status-started = Démarrée
activity-status-completed = Terminée
activity-status-failed = En échec
activity-status-interrupted = Interrompue
activity-outcome-success = Succès
activity-outcome-completed-with-errors = Terminée avec des erreurs
activity-outcome-failed = En échec
activity-outcome-observed = Observé
activity-outcome-proposed = Proposé
activity-summary-scan = Analyse : { $counts }
activity-summary-file-delete = Fichier supprimé
activity-summary-document-merge = Documents fusionnés
activity-summary-document-content-removed = Contenu retiré : { $counts }
activity-summary-metadata-edit = Métadonnées modifiées
activity-summary-tag-edit = Étiquettes modifiées
activity-summary-status-edit = Statut de lecture modifié
activity-summary-cover-edit = Couverture modifiée
activity-summary-missing-file-maintenance = Nettoyage des fichiers manquants : { $counts }
