...
pub enum MuPdfViewerOutput {
    ...
    /// A thumbnail became available for the page at this index.
    NavSubEntryIconUpdated(usize),
}

impl std::fmt::Debug for MuPdfViewerOutput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Close(fp, page) => f.debug_tuple("Close").field(fp).field(page).finish(),
            Self::NavSubEntriesChanged => write!(f, "NavSubEntriesChanged"),
            Self::NavSubEntryActivated(idx) => {
                f.debug_tuple("NavSubEntryActivated").field(idx).finish()
            }
            Self::NavSubEntryIconUpdated(idx) => {
                f.debug_tuple("NavSubEntryIconUpdated").field(idx).finish()
            }
        }
    }
}
...
impl Page for MuPdfViewer {
    ...
    fn nav_sub_entries(&self) -> Vec<crate::page::NavSubEntry> {
        self.pages
            ...
            .enumerate()
            .map(|(idx, page)| {
                let icon = page.icon_handle.as_ref().map(|handle| {
                    cosmic::widget::icon::icon(cosmic::widget::icon::Handle {
                        symbolic: false,
                        data: cosmic::widget::icon::Data::Image(handle.clone()),
                    })
                });
                crate::page::NavSubEntry {
    ...
    fn on_nav_sub_entry_selected(&mut self, index: usize) -> Task<Action<MuPdfViewerMessage>> {
        self.update(MuPdfViewerMessage::SelectPage(index))
    }
    ...
    fn update(&mut self, message: MuPdfViewerMessage) -> Task<Action<MuPdfViewerMessage>> {
        match message {
            ...
            MuPdfViewerMessage::PageDiscovered(layout_gen, page) => {
                ...
                // Display lists are generated by start_discovery() in the same
                // blocking task; no per-page file open needed here.
                Task::done(cosmic::action::app(MuPdfViewerMessage::Out(
                    MuPdfViewerOutput::NavSubEntriesChanged,
                )))
            }
            ...
            MuPdfViewerMessage::ThumbnailReady(layout_gen, pdf_index, handle) => {
                ...
                if let Some(idx) = self.page_index_by_pdf_index(pdf_index) {
                    ...
                    return Task::done(cosmic::action::app(MuPdfViewerMessage::Out(
                        MuPdfViewerOutput::NavSubEntryIconUpdated(idx),
                    )));
                }
                Task::none()
            }
            ...
            MuPdfViewerMessage::SelectPage(idx) => {
                if idx < self.pages.len() {
                    ...
                    return Task::batch([
                        ...
                        Task::done(cosmic::action::app(MuPdfViewerMessage::Out(
                            MuPdfViewerOutput::NavSubEntryActivated(self.active_page),
                        ))),
                    ]);
                }
                Task::none()
