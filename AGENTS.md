...
- **No committed design-spec docs**: brainstorming/planning-skill output (e.g. `docs/superpowers/specs/*.md`) must NOT be committed — these snapshot a design at one point in time and go stale as the code evolves. Keep the design record in the conversation only; delete any such files before committing instead of checking them in.
- **Docs reflect current state only**: committed docs (AGENTS.md, README, module docs) describe the repo as it is now, not as it was implemented. No version numbers, dates, "as of", "recently added", "new in", or migration narratives. No implementation-diary context (why a PR did X, what changed from Y) — that belongs in commit messages/PR descriptions, not docs. Write as a snapshot that stays true regardless of when it's read; update in place rather than appending history when things change. Exception: CHANGELOG.md and release notes — history is their point.
...
- **Commit messages**: concise, no co-author line, never mention Claude or Codex.
